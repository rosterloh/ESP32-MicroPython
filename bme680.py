...
class BME680:
    """Driver for I2C connected BME680 air quality sensor
       :param int address: I2C device address
       :param bool debug: Print debug statements when True.
       :param int refresh_rate: Maximum number of readings per second. Faster property reads
         will be from the previous reading."""

    def __init__(self, address=0x76, debug=False, *, refresh_rate=10):
        """Initialise the I2C device at the 'address' given"""
        self._i2c = machine.I2C(-1, scl=machine.Pin(19), sda=machine.Pin(23))
    ...
    def _read(self, register, length):
        ...
        self._i2c.writeto(self._device_address, bytes([register & 0xFF]))
        result = bytearray(length)
        self._i2c.readfrom_into(self._device_address, result)
        if self._debug:
    ...
    def _write(self, register, values):
        ...
        for i, value in enumerate(values):
            ...
            buffer[2 * i + 1] = value
        self._i2c.writeto(self._device_address, buffer)
        if self._debug:
